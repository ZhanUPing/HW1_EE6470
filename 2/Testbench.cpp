...
void Testbench::do_sobel()
{
  ...
  for (y = 0; y != height; ++y)
  {
    for (x = 0; x != width; ++x)
    {
      ...
      // cout << "Now at " << x << endl;
      if (i_avg_r.num_available() == 0)
        wait(i_avg_r.data_written_event());
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 2) = i_avg_r.read();
      if (i_avg_g.num_available() == 0)
        wait(i_avg_g.data_written_event());
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 1) = i_avg_g.read();
      if (i_avg_b.num_available() == 0)
        wait(i_avg_b.data_written_event());
      *(target_bitmap + bytes_per_pixel * (width * y + x) + 0) = i_avg_b.read();
      // cout << "Now at " << sc_time_stamp() << endl; //print current sc_time
    }
  }
  cout << "number of transfer " << count << endl;
  sc_stop();
}
